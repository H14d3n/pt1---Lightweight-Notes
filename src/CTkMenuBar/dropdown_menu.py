    ...
    def _checkIfMouseLeft(self, event: tk.Event=None) -> None:
        ...
        try:
            if not self._get_coordinates(event.x_root, event.y_root):
                if isinstance(self.menu_seed_object, _CDMSubmenuButton) and hasattr(self.menu_seed_object, 'parent_menu') and self.menu_seed_object.parent_menu.winfo_exists():
                    if not self.menu_seed_object.parent_menu._get_coordinates(event.x_root, event.y_root):
